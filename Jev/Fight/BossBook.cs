...
namespace TerraBlind
{
	...
	public static class BossBook
	{
		...
		// 【读身上真有什么,不枚举饰品】。模组饰品也得写这些字段,否则 vanilla 的移动代码认不出它们
		public static string AbilitiesOf(Terraria.Player p)
		{
			var sb = new System.Text.StringBuilder();
			if (p.slowFall)
				sb.Append("身上有羽落效果:不按上下键时下落速度只有平常的三分之一,按住上键只有十分之一 -- ")
				  .Append("等于能在空中悬停,贴着地面冲过来的东西这样就撞不到。按下键恢复正常下落速度。");
			if (p.wingTimeMax > 0)
				sb.Append($"有翅膀,满格能飞 {p.wingTimeMax / 60} 秒左右,这一刻还剩 {(int)(p.wingTime / 60f)} 秒,落地回满。");
			if (p.dashType != 0)
				sb.Append("有冲刺:朝当前移动方向猛冲一小段,自带冷却,撞到敌人还能免掉那一下伤害。");
			if (p.AnyExtraJumpUsable())
				sb.Append("此刻还有一次空中跳可以用。");
			sb.Append("还有钩爪:甩出去勾住方块,【勾住之后一定要马上跳一次】把自己弹开,")
			  .Append("不跳就会被慢慢拉过去,那不是位移是送死。跳出来能拿到一段速度,而且空中跳会重置。")
			  .Append("钩爪主要用来急转向和急拔高:往上勾、勾住就跳、同时按住上键能窜得很高,")
			  .Append("往左下右下勾则能快速换到另一个高度。荡出去的那一段改不了方向,吊着不跳也打不到人。");
			return sb.ToString();
		}
			...
			new ModBoss
			{
				...
				Info = new BossInfo
				{
					...
					HowItFights =
						...
						+ "所以最该盯的永远是头在哪、朝哪走。"
						+ "【这一场没有安全距离,也没有站得住的地方】,它的速度远超跑动,"
						+ "拉开多远都会被追上,停下来更是直接被啃 -- 能救命的只有一直在动。"
						+ "【这一场冲刺期间人是无敌的】:整段冲刺里撞到它的头和身体都不掉血。"
						+ "所以头扑过来的那一下冲出去反而是最安全的,"
						+ "平时那种'乱冲会撞进攻击里'的顾虑在这一场不成立。"
						+ "冲刺是留给头贴近的那一瞬间的。头在远处、或者它正绕着圈走的时候,"
						+ "冲刺只是把冷却浪费掉,等头真的扑过来时反而没得冲了。"
						+ "冲刺的方向要和头的来向成一个大角度:冲刺只有左右,"
						+ "所以迎着它来的方向冲,同时让自己在升或者降 -- "
						+ "两个方向合起来才是斜着切开它的路线。顺着它来的方向跑是跟它抢同一条线。"
						+ "它会绕着人盘旋,把人圈在中间,那时候四周都是体节,冲刺能从体节之间穿出去 -- "
						+ "看 boss_pieces_around_me,哪一侧的数最小就往哪一侧钻。"
						+ "冲刺挡得住撞击,挡不住弹幕。"
						+ "【没在冲刺的时候撞到体节照样掉血】,竖直掉下去穿过一串体节就是白送几下,"
						+ "要穿过去就得是冲刺着穿。"
						+ "场上会出现激光的预警线:incoming_projectiles 里 is_warning_line 为真的就是,"
						+ "cells_wide 和 cells_tall 说明它横着还是竖着。线亮起之后过一会儿才真发射,"
						+ "那段时间正好用来挪到两条线之间的空隙里。",
				},
			},
		};
