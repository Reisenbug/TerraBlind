		...
		static readonly Dictionary<int, BossInfo> Book = new()
		{
			...
			[Terraria.ID.NPCID.Spazmatism] = new BossInfo
			{
				...
				HowItFights =
					...
					+ "一阶段的魔焰眼追着人飞:【上下不能停】。它贴着自己的高度扑过来,"
					+ "一直在换高度它就一直要重新拐上来,而横着跑是跟它比速度,比不过。"
					+ "只有它已经起冲、那条直线压到身上时才横向闪一步,冲刺锁的是起冲那一刻的位置。"
					+ "它掉到一半血会脱壳进二阶段,那之后的循环是:【喷一段时间火,然后连冲六次,再重来】。"
					+ "喷火伤害很高但只在近处才够得着,所以二阶段要比一阶段站得更远一点 --"
					+ "但不能远到打不中,看 percent_of_my_usual_damage_right_now 确认伤害还在出。"
					+ "那六次冲刺的碰撞比喷火还疼,基本不能吃到,靠上下换层让它扑空。"
					+ "【怎么看出它在冲】:boss_is_charging 为真、或者 boss_speed_cells_per_second"
					+ "接近 boss_top_speed_this_fight,就是那六次冲刺里的一次 --"
					+ "速度慢下来说明这一轮冲完了,那是拉开距离和输出的空档。"
					+ "【魔焰眼的火会点燃自己】,烧起来之后一直在掉血,所以吃一发火的代价比伤害数字大得多。"
					+ "【激光眼最危险的是冲撞不是激光】:激光打在身上不算什么,但它撞过来一下就是一大块血 --"
					+ "所以两只眼睛都要当成会撞人的东西躲,不能因为在打魔焰眼就放着激光眼不管。"
