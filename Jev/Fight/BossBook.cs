		...
		static readonly Dictionary<int, BossInfo> Book = new()
		{
			...
			[Terraria.ID.NPCID.Spazmatism] = new BossInfo
			{
				...
				HowItFights =
					...
					+ "【碰到它掉的血比吃弹幕多得多】,所以躲开碰撞永远优先于其他考虑。"
					+ "它追着人飞,【躲它主要靠上下走】:它贴着自己的高度扑过来,"
					+ "换一层高度它就要重新拐上来,而横着跑是跟它比速度,比不过。"
					+ "只有它已经起冲、那条直线压到身上的那一下,才需要横向闪开 --"
					+ "冲刺锁的是起冲那一刻的位置,横向一步就能让整条线扑空。"
					+ "所以顺序是:先用上下拉开,冲撞压到眼前时再横闪。"
					+ "它掉到一半血会脱掉外壳进二阶段,那之后不再吐弹,改成一轮接一轮地直冲 --"
					+ "【二阶段全程保持距离】,冲刺结束它要拐回来重新加速,那个空档就是拉开的时候。"
					+ "【场上始终是两只眼睛,躲的时候两只都要算】。武器只打魔焰眼,"
					+ "但激光眼同时在从远处直线打激光,other_enemies 里能看到它在哪 --"
					+ "背对一只跑常常正好撞进另一只,往两只都不在的那一侧走才是真的躲开。"
					+ "和任何一只连成水平线都会吃激光,高度和它们都错开的位置最安全。",
			},
