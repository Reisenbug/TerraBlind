		...
		static readonly Dictionary<int, BossInfo> Book = new()
		{
			...
			[Terraria.ID.NPCID.Spazmatism] = new BossInfo
			{
				...
				HowItFights =
					...
					+ "它有两种威胁,要躲的方向不一样。隔着距离的时候它吐绿色的诅咒火焰弹,"
					+ "那是一发一发飞过来的,横着跑就能从弹道之间穿出去,这时候上下动反而慢。"
					+ "贴近之后它改成拖着火焰直冲,冲刺沿着一条直线,锁的是起冲那一刻自己在的位置 --"
					+ "这一下横着跑是跟它抢同一条线,而换个高度能让整条线扑空。"
					+ "所以先看它在不在冲刺、离得多近,再决定这一下是横着躲还是上下躲。"
					+ "冲刺结束它要拐回来重新加速,那个空档是拉开距离的时候。"
